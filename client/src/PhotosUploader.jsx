...
const PhotosUploader = ({ state, setState }) => {
  ...
  const removePhoto = (link) => {
    setState({
      ...state,
      addedPhotos: [...state.addedPhotos.filter((photo) => photo !== link)],
    });
  };

  function selectAsMainPhoto(link) {
    const addedPhotosWithoutSelected = state.addedPhotos.filter(
      (photo) => photo !== link
    );
    const newAddedPhotos = [link, ...addedPhotosWithoutSelected];
    setState({ ...state, addedPhotos: newAddedPhotos });
  }
            ...
            return (
              <div key={index} className="h-32 flex relative">
                ...
                <button
                  ...
                  className="absolute bottom-1 right-1 text-primary bg-opacity-50 rounded-2xl bg-white py-2 px-3 cursor-pointer"
                >
                  <svg
                ...
                <button
                  ...
                  className="absolute bottom-1 left-1 text-white bg-opacity-50 rounded-2xl bg-black py-2 px-3 cursor-pointer"
                >
                  {state.addedPhotos[0] === link ? (
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="currentColor"
